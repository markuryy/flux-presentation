...
const VAEVisualization: React.FC<VAEVisualizationProps> = ({ width = 300 }) => {
  ...
  return (
    <svg width={svgWidth} height={svgHeight} viewBox={`0 0 ${svgWidth} ${svgHeight}`}>
      {/* Container */}
      <rect x="0" y="0" width={svgWidth} height={svgHeight} fill="#f9f9f9" rx={15 * scale} ry={15 * scale} />

      {/* Latent space */}
      <rect x={(svgWidth - latentSize) / 2} y={containerPadding} width={latentSize} height={latentSize} fill="#e6f2ff" rx={cornerRadius} ry={cornerRadius} />
      <text x={svgWidth / 2} y={latentSize / 2 + containerPadding} textAnchor="middle" dominantBaseline="middle" fill="#4d4d4d" fontSize={12 * scale}>64x64 Latent</text>
      ...
      <path 
        ...
        fill="#f0e6ff"
      />
      <text x={svgWidth / 2} y={latentSize + spacing + vaeHeight / 2 + containerPadding} textAnchor="middle" dominantBaseline="middle" fill="#4d4d4d" fontSize={12 * scale}>VAE</text>

      {/* Output image */}
      <rect x={containerPadding} y={totalHeight - imageSize + containerPadding} width={imageSize} height={imageSize} fill="#e6ffe6" rx={cornerRadius} ry={cornerRadius} />
      <text x={svgWidth / 2} y={totalHeight - imageSize / 2 + containerPadding} textAnchor="middle" dominantBaseline="middle" fill="#4d4d4d" fontSize={12 * scale}>1024x1024 Image</text>
      ...
      <path 
        ...
        fill="none" stroke="#99ccff" strokeWidth={2 * scale} markerEnd="url(#arrowhead)"
      />
      <path 
      ...
      <defs>
        <marker id="arrowhead" markerWidth={10 * scale} markerHeight={7 * scale} refX={9 * scale} refY={3.5 * scale} orient="auto">
          <polygon points={`0 0, ${10 * scale} ${3.5 * scale}, 0 ${7 * scale}`} fill="#99ccff" />
        </marker>
      </defs>
    </svg>
  )
